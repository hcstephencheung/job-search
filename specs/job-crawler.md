...
- **Locations:** Metro Vancouver, BC, or Remote within Canada; also remote US or North America roles at companies with an engineering team in Canada (see Location in the Algorithm spec)
...
- **Each job:** `rank`, `title`, `url` (required); `company`, `board`, `location`, `remote` (true/false), `postedDate` (YYYY-MM-DD), `salary` {`min`, `max`, `currency`}, `stackMatch` (list), `description` (max 1,200 chars), `canadianEng` (evidence links, only on jobs qualified by the Canadian engineering check)
- **slugs/\<slug>:\<jobBoard>** (one doc per company-board pair, written by slug discovery): `slug`, `jobBoard`, `companyName`
- **companies/\<slug>** (one doc per company checked for a Canadian engineering team): `companyName`, `linkedinSlug`, `canadianEng` (true, false or unknown), `evidence` (profile or posting URLs), `checkedAt` (YYYY-MM-DD)
